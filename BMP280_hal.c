...
HAL_StatusTypeDef BMP280_normal_measure() {
	...
	// Можно было бы сразу в return засунуть Mem_Write
...
HAL_StatusTypeDef BMP280_get_measure(double *temp, double *press, double *h) {
	...
	*press = __BMP280_compensate_P_int64(ADC_data[1])/256.;
	// Константы собраны в одну, может место освободит чуть-чуть
	// Температуру надо из градусов Цельсия в градусы Кельвина переводить
	*h = 29.254 * ((*temp) + 273.15) * log(refPressure/(*press));
	return HAL_OK;
}
