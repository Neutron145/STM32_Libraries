...
HAL_StatusTypeDef BMP280_init(I2C_TypeDef *I2Cx_, uint32_t refPressure_) {
	...
	//HAL_I2C_Mem_Read(&hi2c_, BMP280_ADDRESS, BMP280_REGISTER_ID, I2C_MEMADD_SIZE_8BIT, &id, 1, 0xFF);
	if(id == 0x60) {
		...
		//HAL_I2C_Mem_Read(&BMP280_hi2c, BMP280_ADDRESS, BMP280_REGISTER_CALIBRATION, I2C_MEMADD_SIZE_8BIT, (uint8_t*) &BMP280_calibration_data, 24, 0xFF);
...
HAL_StatusTypeDef BMP280_config(uint8_t T_OS, uint8_t P_OS, uint8_t STDB, uint8_t IIRF) {
	...
	return HAL_OK;
	//return HAL_I2C_Mem_Write(&BMP280_hi2c, BMP280_ADDRESS, BMP280_REGISTER_CTRL_MEAS, I2C_MEMADD_SIZE_8BIT, (uint8_t*)&BMP280_sensor_settings, 2, 0xFF);
}
...
HAL_StatusTypeDef BMP280_forced_measure(float *temp, float *press, float *h) {
	...
	I2C_write_bytes(BMP280_I2C, BMP280_ADDRESS, BMP280_REGISTER_CTRL_MEAS, (uint8_t*)&BMP280_sensor_settings.ctrl_meas, 2);
	//if(HAL_I2C_Mem_Write(&BMP280_hi2c, BMP280_ADDRESS, BMP280_REGISTER_CTRL_MEAS, I2C_MEMADD_SIZE_8BIT, (uint8_t*)&BMP280_sensor_settings.ctrl_meas, 1, 0xFF) != HAL_OK) {
	//	return HAL_ERROR;
	//}
	uint8_t status = 1;
	while(status >> 3 != 0) {
		I2C_write_bytes(BMP280_I2C, BMP280_ADDRESS, BMP280_REGISTER_STATUS, &status, 1);
		//if(HAL_I2C_Mem_Read(&BMP280_hi2c, BMP280_ADDRESS, BMP280_REGISTER_STATUS, I2C_MEMADD_SIZE_8BIT, &status, 1, 0xFF) != HAL_OK){
		//	return HAL_ERROR;
		//}
	}
	...
	//if(HAL_I2C_Mem_Read(&BMP280_hi2c, BMP280_ADDRESS, BMP280_REGISTER_RAW_DATA, I2C_MEMADD_SIZE_8BIT, raw_data, 6, 0xFF) != HAL_OK) {
	//	return HAL_ERROR;
	//}
	int32_t ADC_data[2];
	for(int i = 0; i < 2; i++) {
