...
HAL_StatusTypeDef I2C__wait_flag(I2C_TypeDef *I2Cx, uint8_t bit, uint8_t timeout) {
	...
	while((I2Cx->SR1 & bit) != bit) {
		...
		if(HAL_GetTick() - start_wait > timeout) {
			if((I2Cx->SR2 & I2C_SR2_BUSY) == I2C_SR2_BUSY) return HAL_BUSY;
			return HAL_ERROR;
		}
	}
	return HAL_OK;
...
HAL_StatusTypeDef LL_I2C_Master_Receive(I2C_TypeDef *I2Cx, uint8_t device_address, uint8_t *buffer, uint8_t buffer_size, uint8_t timeout) {
	...
	if((status = I2C__wait_flag(I2Cx, I2C_SR1_SB, timeout)) != HAL_OK) return status;
	(void) I2Cx->SR1;
	...
	if((status = I2C__wait_flag(I2Cx, I2C_SR1_ADDR, timeout)) != HAL_OK) return status;
	LL_I2C_ClearFlag_ADDR(I2Cx);
	uint16_t i;
	//Сохраняем полученные байты от устройства в буфер
	for(i=0;i<buffer_size-1;i++)
	{
	  if((status = I2C__wait_flag(I2Cx, I2C_SR1_RXNE, timeout)) != HAL_OK) return status;
...
HAL_StatusTypeDef LL_I2C_Master_Transmit(I2C_TypeDef *I2Cx, uint8_t device_address, uint8_t *buffer, uint8_t buffer_size, uint8_t timeout) {
	...
	if((status = I2C__wait_flag(I2Cx, I2C_SR1_ADDR)) != HAL_OK) return status;
	(void) I2Cx->SR1;
	(void) I2Cx->SR2;
	...
	for(uint16_t i = 0; i < buffer_size; i++) {
		...
		if((status = I2C__wait_flag(I2Cx, I2C_SR1_TXE, timeout)) != HAL_OK) return status;
	}
