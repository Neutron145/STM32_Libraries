...
HAL_StatusTypeDef LSM6DS33_init(LSM6DS33_handler* handler, I2C_HandleTypeDef* hi2c_) {
	...
	if(handler->id == 0x69) {
		handler->hi2c_ = hi2c_;
		handler->config.ORIENT_config = 0b0;
		handler->config.CTRL1_config;
		handler->config.CTRL2_config = 0b0;
		handler->config.CTRL3_config = 0b01000100;
		handler->config.CTRL7_config = 0b11101000;
		handler->config.CTRL8_config = 0b11000000;
		handler->config.CTRL10_config = 0b00010000;

		//!< Включаем фильтр для акселерометра
		if((status = I2C_Mem_Write(hi2c_, LSM6DS33_ADDRESS, LSM6DS33_REGISTER_CTRL8, &handler->config.CTRL8_config, 1, 0xFF)) != HAL_OK) {
			return status;
		}

		//!< Включаем фильтр
		if((status = I2C_Mem_Write(hi2c_, LSM6DS33_ADDRESS, LSM6DS33_REGISTER_TAP_CFG, &handler->config.CTRL10_config, 1, 0xFF)) != HAL_OK) {
			return status;
		}

		//!< Включаем HPF для гироскопа
		if((status = I2C_Mem_Write(hi2c_, LSM6DS33_ADDRESS, LSM6DS33_REGISTER_CTRL7, &handler->config.CTRL7_config, 1, 0xFF)) != HAL_OK) {
			return status;
		}
	...
	for(int i = 0; i < 3; i++) {
		handler->calibration_data.a_bias[i] = 0;
		handler->calibration_data.g_bias[i] = 0;
	}
...
HAL_StatusTypeDef LSM6DS33_config_orientation(LSM6DS33_handler* handler, uint8_t orient, uint8_t signs) {
	...
	if((status = I2C_Mem_Write(handler->hi2c_, handler->address, LSM6DS33_REGISTER_ORIENT_CFG, &handler->config.ORIENT_config, 1, 0xFF)) != HAL_OK) {
		return status;
	}
	return HAL_OK;
}
...
HAL_StatusTypeDef LSM6DS33_config_filters(LSM6DS33_handler* handler, uint8_t g_HPF, uint8_t g_HPF_frequency, uint8_t a_HPF) {
	...
	__LSM6DS33_modify_reg(&handler->config.CTRL8_config, LSM6DS33_A_FILTER_MASK, a_HPF << 5);
	
	if((status = I2C_Mem_Write(handler->hi2c_, handler->address, LSM6DS33_REGISTER_CTRL7, &handler->config.CTRL7_config, 2, 0xFF)) != HAL_OK) {
		return status;
	}
...
HAL_StatusTypeDef LSM6DS33_config_full_scale(LSM6DS33_handler* handler, uint8_t a_FS, uint8_t g_FS) {
	...
	__LSM6DS33_modify_reg(&handler->config.CTRL1_config, LSM6DS33_FULL_SCALE_MASK, a_FS << 2);
	__LSM6DS33_modify_reg(&handler->config.CTRL2_config, LSM6DS33_FULL_SCALE_MASK, g_FS << 2);
	if((status = I2C_Mem_Write(handler->hi2c_, handler->address, LSM6DS33_REGISTER_CTRL1, &handler->config.CTRL1_config, 2, 0xFF)) != HAL_OK) {
		return status;
	}
...
HAL_StatusTypeDef LSM6DS33_config_perfomance_mode(LSM6DS33_handler* handler, uint8_t a_ODR, uint8_t g_ODR) {
	...
	__LSM6DS33_modify_reg(&handler->config.CTRL1_config, LSM6DS33_ODR_MASK, a_ODR << 4);
	__LSM6DS33_modify_reg(&handler->config.CTRL2_config, LSM6DS33_ODR_MASK, g_ODR << 4);
...
HAL_StatusTypeDef LSM6DS33_reset(LSM6DS33_handler* handler) {
	...
	if((status = I2C_Mem_Write(handler->hi2c_, handler->address, LSM6DS33_REGISTER_CTRL3, &handler->config.CTRL3_config, 1, 0xFF)) != HAL_OK) {
		return status;
	}
