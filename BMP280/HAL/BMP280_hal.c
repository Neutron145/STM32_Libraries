...
BMP280_calibration_data calibration_data;
 ...
 */
HAL_StatusTypeDef BMP280_init(I2C_HandleTypeDef *hi2c_, uint32_t ref_pressure_) {
	uint8_t id;
	...
	if (id == 0x58) {
		BMP280_hi2c = hi2c_;
		BMP280_ref_pressure = ref_pressure_;

		status = HAL_I2C_Mem_Read(BMP280_hi2c, BMP280_ADDRESS, BMP280_REGISTER_CALIBRATION, I2C_MEMADD_SIZE_8BIT, (uint8_t*) &BMP280_calibration_data, 24, 0xFF);
...
HAL_StatusTypeDef BMP280_forced_measure(float *temp, float *press, float *h) {
	...
	if ((status = HAL_I2C_Mem_Write(BMP280_hi2c, BMP280_ADDRESS, BMP280_REGISTER_CTRL_MEAS, I2C_MEMADD_SIZE_8BIT, (uint8_t*)&BMP280_sensor_settings.ctrl_meas, 1, 0xFF)) != HAL_OK) {
		return status;
	}

	uint8_t status_of_measure = 1;
	while (status_of_measure >> 3 != 0) {
		if ((status = HAL_I2C_Mem_Read(BMP280_hi2c, BMP280_ADDRESS, BMP280_REGISTER_STATUS, I2C_MEMADD_SIZE_8BIT, &status, 1, 0xFF)) != HAL_OK) {
			return status;
		}
	}

	uint8_t raw_data[6];
	if ((status = HAL_I2C_Mem_Read(BMP280_hi2c, BMP280_ADDRESS, BMP280_REGISTER_RAW_DATA, I2C_MEMADD_SIZE_8BIT, raw_data, 6, 0xFF)) != HAL_OK) {
		return status;
	}
