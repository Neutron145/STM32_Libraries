...
HAL_StatusTypeDef LSM6DS33_config_orientation(uint8_t orient, uint8_t signs) {
	...
	if(HAL_I2C_Mem_Write(&LSM6DS33_hi2c, LSM6DS33_ADDRESS, LSM6DS33_REGISTER_ORIENT_CFG, I2C_MEMADD_SIZE_8BIT, &LSM6DS33_config.ORIENT_config, 1, 0xFF) != HAL_OK) {
		return HAL_ERROR;
	}
	return HAL_OK;
}
...
HAL_StatusTypeDef LSM6DS33_config_filters(uint8_t g_HPF, uint8_t g_HPF_frequency, uint8_t a_HPF) {
	...
	if(HAL_I2C_Mem_Write(&LSM6DS33_hi2c, LSM6DS33_ADDRESS, LSM6DS33_REGISTER_CTRL7, I2C_MEMADD_SIZE_8BIT, &LSM6DS33_config.CTRL7_config, 2, 0xFF) != HAL_OK) {
		return HAL_ERROR;
	}
	return HAL_OK;
}
...
HAL_StatusTypeDef LSM6DS33_config_full_scale(uint8_t a_FS, uint8_t g_FS) {
	...
	if(HAL_I2C_Mem_Write(&LSM6DS33_hi2c, LSM6DS33_ADDRESS, LSM6DS33_REGISTER_CTRL1, I2C_MEMADD_SIZE_8BIT, &LSM6DS33_config.CTRL1_config, 2, 0xFF) != HAL_OK) {
		return HAL_ERROR;
	}
	return HAL_OK;
}
...
HAL_StatusTypeDef LSM6DS33_reset() {
	...
	if(HAL_I2C_Mem_Write(&LSM6DS33_hi2c, LSM6DS33_ADDRESS, LSM6DS33_REGISTER_CTRL3, I2C_MEMADD_SIZE_8BIT, &LSM6DS33_config.CTRL3_config, 1, 0xFF) != HAL_OK) {
		return HAL_ERROR;
	}
	return HAL_OK;
}
...
HAL_StatusTypeDef LSM6DS33_A_get_measure(float *a) {
	uint16_t raw_data[3];
	if(HAL_I2C_Mem_Read(&LSM6DS33_hi2c, LSM6DS33_ADDRESS, LSM6DS33_REGISTER_OUT_A, I2C_MEMADD_SIZE_8BIT, (uint8_t*)&raw_data, 6, 0xFF) != HAL_OK) {
		return HAL_ERROR;
	}
	for(int i = 0; i < 3; i++) {
...
HAL_StatusTypeDef LSM6DS33_G_get_measure(float *g) {
	uint16_t raw_data[3];
	if(HAL_I2C_Mem_Read(&LSM6DS33_hi2c, LSM6DS33_ADDRESS, LSM6DS33_REGISTER_OUT_G, I2C_MEMADD_SIZE_8BIT, (uint8_t*)&raw_data, 6, 0xFF) != HAL_OK) {
		return HAL_ERROR;
	}
	for(int i = 0; i < 3; i++) {
...
HAL_StatusTypeDef LSM6DS33_get_measure(float* a, float *g) {
	uint16_t raw_data[6];
	if(HAL_I2C_Mem_Read(&LSM6DS33_hi2c, LSM6DS33_ADDRESS, LSM6DS33_REGISTER_OUT_G, I2C_MEMADD_SIZE_8BIT, (uint8_t*)&raw_data, 12, 0xFF) != HAL_OK) {
		return HAL_ERROR;
	}
	for(int i = 0; i < 3; i++) {
	  g[i] = ((float) raw_data[i])*FULL_SCALES_G[(LSM6DS33_config.CTRL2_config & LSM6DS33_FULL_SCALE_MASK) >> 2]/(float)0x8000;
