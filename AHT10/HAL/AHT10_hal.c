...
HAL_StatusTypeDef AHT10_init(I2C_HandleTypeDef hi2c_) {
	if(HAL_I2C_Master_Transmit(&hi2c_, AHT10_ADDRESS, (uint8_t*)AHT10_COMMAND_INIT, 1, 0xFF) == HAL_OK) {
		AHT10_hi2c = hi2c_;
		return HAL_OK;
	}
	return HAL_ERROR;
...
HAL_StatusTypeDef AHT10_trigger_measure() {
	if(HAL_I2C_Master_Transmit(&AHT10_hi2c, AHT10_ADDRESS, AHT10_trigger_data, 3, 0xFF) == HAL_OK) {
		return HAL_OK;
	}
	return HAL_ERROR;
}
...
HAL_StatusTypeDef AHT10_get_measure(float *t, float *hum) {
	if(HAL_I2C_Master_Receive(&AHT10_hi2c, AHT10_ADDRESS, AHT10_RxData, 7, 0xFF) == HAL_OK) {
...
HAL_StatusTypeDef AHT10_soft_reset() {
	if(HAL_I2C_Master_Transmit(&AHT10_hi2c, AHT10_ADDRESS, (uint8_t*)AHT10_COMMAND_RESET, 1, 0xFF) == HAL_OK) {
		return HAL_OK;
	}
	return HAL_ERROR;
}
