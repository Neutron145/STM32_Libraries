 ...
 * @retval
 */
void SX1268_init(SPI_TypeDef hspi_) {
	SX1268_hspi = hspi_;
	uint8_t packet_type_op[2] = {SX1268_OP_SET_PACKET_TYPE, 0x01};
	LL_SPI_Transmit(&SX1268_hspi, packet_type_op, 2);
}
...
HAL_StatusTypeDef SX1268_config_rfFreq(uint8_t* rfFreq) {
	...
	return LL_SPI_Transmit(&SX1268_hspi, rf_freq_op);
}
...
HAL_StatusTypeDef SX1268_config_modulation_params(uint8_t* params) {
	...
	return LL_SPI_Transmit(&SX1268_hspi, mod_params_op, 9);
}
...
HAL_StatusTypeDef SX1268_config_baseAddr(uint8_t txAddr, uint8_t rxAddr) {
	...
	return LL_SPI_Transmit(&SX1268_hspi, base_addres_op, 3);
}
...
HAL_Status SX1268_config_tx_params(uint8_t power, unit8_t rampTime) {
	...
	return LL_SPI_Transmit(&SX1268_hspi, tx_params_op, 3);
}
...
HAL_StatusTypeDef SX1268_config_packet_params(uint8_t* params) {
	...
	return LL_SPI_Transmit(&SX1268_hspi, pack_params_op, 9);
}
...
HAL_StatusTypeDef SX1268_send_message(uint8_t* buffer, int buffer_size) {
	...
	if(LL_SPI_Transmit(&SX1268_hspi, write_buffer_op , buffer_size) != HAL_OK){
		return HAL_ERROR;
	}
	if(LL_SPI_Transmit(&SX1268_hspi, buffer, buffer_size) != HAL_OK){
		return HAL_ERROR;
	}
	uint8_t set_tx_op[4] = {SX1268_OP_SET_TX, 0x00, 0x00, 0x00};
	return LL_SPI_Transmit(&SX1268_hspi, set_tx_op , 4);
}
